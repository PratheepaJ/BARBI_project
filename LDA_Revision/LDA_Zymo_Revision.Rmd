...
Looking at the DADA2 pipeline ASVs 2, 3, 4, 5, 6, 7, 8, 11, 17, 37, 22 are in the ZymoBIOMICS® Microbial Community Standard. We used BARBI to identify DNA contaminants. We will determine using LDA whether DNA contamiantion have any latent communities (topics).
...
stan.fit = sampling(f, 
  ...
  iter = iter, 
  chains = 2, 
  sample_file = "sample_file_ER.rds",
  diagnostic_file = "diagnostic_file_ER.rds",
  cores = 2)
...
```{r}
p
ggsave("./Figures/Zymo_LDA_theta.eps", plot = p, width = 12, height = 16)
```
...
p = ggplot(data = theta.all) + 
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 15), 
    ...
    strip.text.x = element_text(size = 12))
p
ggsave("./Figures/Zymo_LDA_theta_col.eps", plot = p, width = 10, height = 6)
```
...
ggheatmap(
  ...
  theme(plot.title = element_text(hjust = 0.5, size = 8), axis.text.x = element_text(size = 8), axis.text.y = element_text(size = 12)) + 
  scale_fill_gradient(low = "white", high = "orange")
```


We used LDA on the standard eight dilution series samples and 10 negative controls. LDA starting with $T=10$ topics on the ZymoBIOMICS data assigned each negative control sample to a different topic. By varying the number of topics, we observed that two topics were consistently assigned to dilution series samples. Thus, we reduced the number of topics to four which showed one clear contaminant topic (Topic 1), two topics dominated by true ASVs (Topic 2 and 4), and a third topic (Topic 3) which was a mixture of true ASVs with low abundance and contaminant ASVs as in Figure \ref{fig10}. 
...
#levels(beta.all$Topic) = c(paste0("Topic_", seq(1,K)))
beta.all$ASV = factor(beta.all$ASV)
#levels(beta.all$ASV) = df.ASV$ASV.Class


max.beta.in.each.asv.all.topics = group_by(beta.all,Topic, Genus) %>% summarise(max_beta = max(ASV.distribution)) %>% top_n(10, max_beta) %>% as.data.frame()

library(ggwordcloud)

p_cloud = ggplot(max.beta.in.each.asv.all.topics, 
                 aes(label = Genus, size = max_beta, color = Genus)) + 
  geom_text_wordcloud() +
  theme_minimal() +
  scale_size_area(max_size = 18) + 
  facet_wrap(~ Topic) + theme(strip.text.x = element_text(size = 18, face = "bold"))

ggsave("./Figures/Zymo_LDA_cloud.png", p_cloud, width = 20, height = 15, units = "in", dpi = 300)
```
