...
p.all.blocks = lapply(theta.all.blocks, function(u){
  sel.samples = c(as.character(unique(u$Sample[(u$Sample_Type == "Control")])), 
                  as.character(unique(u$Sample[(u$Sample_Type == "Plasma")])[c(1:10, 48)]))
...
```{r}
p.all.blocks[[1]]
ggsave("./Figures/SIRS_LDA_theta_set1.eps", plot = p.all.blocks[[1]], width = 9, height = 10)
```


```{r}
p.all.blocks[[2]]
ggsave("./Figures/SIRS_LDA_theta_set2.eps", plot = p.all.blocks[[2]], width = 9, height = 10)
```

```{r}
p.all.blocks[[3]]
ggsave("./Figures/SIRS_LDA_theta_set3.eps", plot = p.all.blocks[[3]], width = 9, height = 10)
```
...
```{r}
library(ggwordcloud)
max.beta.in.each.asv.all.topics.all.blocks = lapply(as.list(1:3), function(i){
...
ggsave("./Figures/SIRS_Set1_ASV.png", p_cloud_all_blocks[[1]], width = 15, height = 10, units = "in", dpi = 300)
```

```{r fig.width=15, fig.height=10}
p_cloud_all_blocks[[2]]
ggsave("./Figures/SIRS_Set2_ASV.png", p_cloud_all_blocks[[2]], width = 15, height = 10, units = "in", dpi = 300)
```

```{r fig.width=15, fig.height=10}
p_cloud_all_blocks[[3]]

ggsave("./Figures/SIRS_Set3_ASV.png", p_cloud_all_blocks[[3]], width = 15, height = 10, units = "in", dpi = 300)
```
