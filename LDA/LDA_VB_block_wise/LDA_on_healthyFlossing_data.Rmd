---
title: "Healthy flossing data - LDA (Variational Bayes) analysis"
...
saveRDS(stan.fit.all.blocks, file = "./Results/stan_fit_all_HF.rds")
```

```{r}
stan.fit.all.blocks <- readRDS("./Results/stan_fit_all_HF.rds")
```
...
```{r}
p.all.blocks[[1]]
ggsave("./Figures/LDA_HF_set1_2.eps", plot = p.all.blocks[[1]], width = 12, height = 16)
```

```{r}
p.all.blocks[[2]]
ggsave("./Figures/LDA_HF_set2_2.eps", plot = p.all.blocks[[2]], width = 12, height = 16)
```
`
...
```{r}
p.all.blocks[[1]]
ggsave("./Figures/LDA_HF_set1_boxplot.eps", plot = p.all.blocks[[1]], width = 9, height = 6)
```


```{r}
p.all.blocks[[2]]
ggsave("./Figures/LDA_HF_set2_boxplot.eps", plot = p.all.blocks[[2]], width = 9, height = 6)
```
...
<!-- If there are contaminant species that were still present after using BARBI, we can use LDA to determine the topic that those species belongs to. This will help to remove more contaminant species. -->
