...
saveRDS(stan.fit.all.blocks, file = "stan_fit_all_HF.rds")
```
...
p.all.blocks = lapply(theta.all.blocks, function(u){
  ...
  p = ggplot() + 
    ...
    theme(plot.title = element_text(hjust = 0.5), 
      strip.text.y= element_text(size = 6, angle = 360), strip.text.x = element_text(size  = 8))
  return(p)
})
...
<!-- If there are contaminant speceis that were still present after using BARBI, we can use LDA to determine the topic that those species belongs to. This will help to remove more contaminant species. -->
