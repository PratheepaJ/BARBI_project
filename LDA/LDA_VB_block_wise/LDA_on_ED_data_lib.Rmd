...
```{r}
lambda.blks = lapply(as.list(seq(1,3)), function(i){
    x = psByBlock[[i]]
...
saveRDS(stan.fit.all.blocks, file = "./Results/stan_fit_all_ED_lib_corr.rds")
```


```{r}
stan.fit.all.blocks = readRDS(file = "./Results/stan_fit_all_ED_lib_corr.rds")
```
...
```{r}
p.all.blocks[[1]]
ggsave("./Figures/LDA_ED_set1_lib_corr.eps", plot = p.all.blocks[[1]], width = 12, height = 16)
```

```{r}
p.all.blocks[[2]]
ggsave("./Figures/LDA_ED_set2_lib_corr.eps", plot = p.all.blocks[[2]], width = 12, height = 16)
```

```{r}
p.all.blocks[[3]]
ggsave("./Figures/LDA_ED_setP_lib_corr.eps", plot = p.all.blocks[[3]], width = 12, height = 16)
```
