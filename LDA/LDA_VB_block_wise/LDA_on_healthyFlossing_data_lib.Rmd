...
```{r}
lambda.blks = lapply(as.list(seq(1,2)), function(i){
    x = psByBlock[[i]]
...
```{r}
f = stan_model(file = "lda_lib_dep_corr.stan")
```
...
saveRDS(stan.fit.all.blocks, file = "./Results/stan_fit_all_HF_lib_corr.rds")
```


```{r}
stan.fit.all.blocks = readRDS(file = "./Results/stan_fit_all_HF_lib_corr.rds")
```
...
```{r}
p.all.blocks[[1]]
ggsave("./Figures/LDA_HF_set1_lib_corr.eps", plot = p.all.blocks[[1]], width = 12, height = 16)
```

```{r}
p.all.blocks[[2]]
ggsave("./Figures/LDA_HF_set2_lib_corr.eps", plot = p.all.blocks[[2]], width = 12, height = 16)
```
`
