...
```{r}
lambda.blks = lapply(as.list(seq(1,3)), function(x){
    x = psByBlock[[1]]
...
saveRDS(stan.fit, file = "./Results/stan_vb_Zymo_lib_corr.rds")
```

```{r}
stan.fit = readRDS("./Results/stan_vb_Zymo_lib_corr.rds")
```
...
We chose three topics because one topic always dominated in dilution series samples. 
...
```{r}
p
ggsave("./Figures/LDA_Zymo_lib_corr.eps", plot = p, width = 12, height = 16)
```
